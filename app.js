...
function render() {
    ...
    $('#gallery-list').innerHTML = d.gallery.map((item, index) => {
        const category = item[1].split(' / ')[0].toLowerCase();
        const codeLine = ['ship --quiet', 'focus --deep', 'learn --in-public', 'craft --details', 'repeat --better'][index];
        return `<figure class="gallery-item reveal" data-memory="${category}"><img loading="lazy" src="${item[2]}" alt="${item[0]}"><div class="gallery-signal mono">0${index + 1} / ${category}</div><figcaption class="gallery-caption"><span>${item[0]}</span><small>${item[1]}</small><code>&gt; ${codeLine}</code></figcaption></figure>`;
    }).join('');
}
...
function initInteractions() {
    ...
    $$('.gallery-filter-btn').forEach(filter => filter.addEventListener('click', () => {
        $$('.gallery-filter-btn').forEach(item => item.classList.remove('active'));
        filter.classList.add('active');
        const category = filter.dataset.galleryFilter;
        const visible = $$('.gallery-item').filter(item => category === 'all' || item.dataset.memory === category);
        $$('.gallery-item').forEach(item => item.classList.toggle('is-hidden', !visible.includes(item)));
        $('#gallery-count').textContent = String(visible.length).padStart(2, '0');
    }));
    ...
    $$('.project-links a, .about-copy .button').forEach(link => link.addEventListener('click', event => {
        if (link.getAttribute('href') === '#') event.preventDefault();
        showToast(link.classList.contains('button') ? 'CV download will be available soon.' : `${link.closest('.project').querySelector('h3').textContent}: link coming soon.`);
    }));
